...
#include <string.h>
#include <util/type.h>
#include <srt/TsMuxer.h>

#include <srt/common.h>
#include <srt/crc.h>
...
TsMuxer::TsMuxer() {
    ...
    m_stream_pid_map[STREAM_TYPE_VIDEO_H264]=_H264_PID;
    m_stream_pid_map[STREAM_TYPE_VIDEO_HEVC]=_H265_PID;
    m_stream_pid_map[STREAM_TYPE_AUDIO_AAC]=_AAC_PID;
    m_stream_pid_map[STREAM_TYPE_AUDIO_OPUS]=_OPUS_PID;
    m_stream_pid_map[STREAM_TYPE_PRIVATE_DATA]=_PRIVATE_PID;


    /**m_stream_pid_map.insert(
			pair<uint8_t, int>(STREAM_TYPE_VIDEO_H264, _H264_PID));
	m_stream_pid_map.insert(
			pair<uint8_t, int>(STREAM_TYPE_VIDEO_HEVC, _H265_PID));
	m_stream_pid_map.insert(
			pair<uint8_t, int>(STREAM_TYPE_AUDIO_AAC, _AAC_PID));
	m_stream_pid_map.insert(
			pair<uint8_t, int>(STREAM_TYPE_AUDIO_OPUS, _OPUS_PID));
	m_stream_pid_map.insert(
            pair<uint8_t, int>(STREAM_TYPE_PRIVATE_DATA, _PRIVATE_PID));**/
	current_index = 0;
...
void TsMuxer::create_ts(TsPes *frame, vector<TsBuffer> *sb) {
	bool first = true;
...
void TsMuxer::encode(uint8_t *p, int32_t plen, int32_t frametype,
	...
	TsPes *frame = (TsPes*) malloc(sizeof(TsPes));
...
void TsMuxer::encodeWithPmt(uint8_t *p, int32_t plen, int32_t frametype,
		int64_t timestamp, TsStream streamType,
		vector<TsBuffer> *sb) {
	encodePmtWithoutData(sb);
...
//TsMuxer::current_index=0;
//void TsMuxer::encodePmt(vector<TsBuffer> *sb) {
//	if (should_create_pat()) {
//		encodePmtWithoutData(sb);
//	}
//}
...
void TsMuxer::encodePmtWithoutData(vector<TsBuffer> *sb){
	current_index=0;
