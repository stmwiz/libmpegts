...
int32_t Tsdemux::decode_unit(uint8_t* data_p,  ts_media_data_callback_I* callback)
{
    int32_t pos = 0;
    int32_t npos = 0;
...
int32_t Tsdemux::decode(SRT_DATA_MSG_PTR data_ptr, ts_media_data_callback_I* callback)
{
    int32_t ret = -1;
    std::string path;
...
void Tsdemux::insert_into_databuf(uint8_t* data_p, size_t data_size,  unsigned short pid) {
    _last_pid = pid;
