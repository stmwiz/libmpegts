...
public:
    ...
    void create_pes(TsPes *frame,uint8_t *p,int32_t plen,int64_t timestamp,TsStream streamType);
    void create_pcr(TsBuffer *sb);
    void create_null(TsBuffer *sb);
    void encode(uint8_t* p,int32_t plen,int64_t timestamp ,TsStream streamType,std::vector<TsBuffer> &sb);
    void encode_with_pmt(uint8_t* p,int32_t plen,int64_t timestamp ,TsStream streamType,std::vector<TsBuffer> &sb);
